...
func LoadConfigOptional(configFile string, optional bool) (*Config, error) {
	...
	if err != nil {
		if optional {
			if os.IsNotExist(err) || errors.Is(err, syscall.EISDIR) {
				...
				cfg := &Config{CredentialInFlight: DefaultCredentialInFlightConfig(), AccountConcurrency: DefaultAccountConcurrencyConfig()}
	...
	if errValidate := validateCredentialWeightYAML(data); errValidate != nil {
		if optional {
			cfgOptional := &Config{CredentialInFlight: DefaultCredentialInFlightConfig(), AccountConcurrency: DefaultAccountConcurrencyConfig()}
